...
namespace TileMap
{
    public class TileMapController : MonoBehaviour
    {
        ...
        public void Start()
        {
            ...
            Vector2 targetTile = new Vector2(10, 10);
            if (tileGrid[(int)startTile.x, (int)startTile.y].terrainType != Tile.TerrainType.Mountains && tileGrid[(int)startTile.x, (int)startTile.y].terrainType != Tile.TerrainType.Water)
            {
                if (tileGrid[(int)targetTile.x, (int)targetTile.y].terrainType != Tile.TerrainType.Mountains && tileGrid[(int)targetTile.x, (int)targetTile.y].terrainType != Tile.TerrainType.Water)
                {
                    for (int row = 0; row < gridHeight; row++)
                    {
                        for (int column = 0; column < gridWidth; column++)
                    ...
                    testPath = Pathfinder.pathfinder.FindPath(possiblePathTiles, startTile, targetTile, false);
                    if (testPath != null)
                    {
                        foreach (Vector2 coord in testPath)
                        {
                            tileGrid[(int)coord.x, (int)coord.y].visualRootObject.transform.position += new Vector3(0, 10, 0);
                        }
                    }
                }
            }
